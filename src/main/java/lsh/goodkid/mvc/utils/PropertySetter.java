...
public class PropertySetter {
    ...
    public static void setProperties(ResultSet rs, Object vo, boolean nullToEmpty, boolean useTrim) throws SQLException {
        ...
        for (int i = 0; i < columnCount; i++) {
            ...
            type.put(columnName.toUpperCase(), columnType);
        }

        // vo 클래스 property 검사
        Class voClass = vo.getClass();
        Method[] methods = voClass.getDeclaredMethods(); // vo클래스의 모든 메소드를 저장
                        ...
                        switch (columnTypeString.toUpperCase()) {
                            ...
                            case "CLOB":
                                if (parameters[0] == String.class) {
                                    String varcharVal = rs.getString(propertyName);
                                    if (varcharVal == null) {
                                        if (nullToEmpty) {
                            ...
                            case "NUMBER":
                                if (parameters[0] == int.class) {
                                    int intVal = rs.getInt(propertyName);
                                    if (intVal == 0) {
                                        if (nullToEmpty) {
